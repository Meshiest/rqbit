...
impl TorrentStateInitializing {
    ...
    pub async fn check(&self) -> anyhow::Result<TorrentStatePaused> {
        ...
        let have_pieces = if self.shared.options.skip_initial_check {
            // Skip hash check — try to restore persisted bitfield first so partial
            // downloads keep their real progress instead of being marked 100%.
            match have_pieces {
                Some(persisted) => {
                    info!("Skipping initial check: restored persisted bitfield");
                    persisted
                }
                None => {
                    // No persisted bitfield — assume all pieces are valid
                    // (used after moving already-verified complete files)
                    info!(
                        "Skipping initial check: no persisted bitfield, assuming all pieces valid"
                    );
